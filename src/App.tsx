...
function App() {
  ...
  const { isLoading, isError, data: questionsObtained, refetch: refetchQuestions, isFetching } = useQuery<Question[]>({
    ...
    queryKey: ["Questions"],
    queryFn: async () => {
      const result = await axios.get<APIQuestionsResponse>(`https://opentdb.com/api.php?amount=${amountOfQuestions}&category=${category}`).then((response) => response.data)
  ...
  return (
    <div data-theme={theme} className='min-h-screen transition-colors duration-700'>
